...
\paragraph{Références} \cite{AlphaGo0} \cite{AlphaGo1} \cite{AlphaGo2} \cite{MachineLearning0} \cite{Internet4} \cite{Internet5}
...
d'appliquer différents algorithmes à un problème, pour le résoudre de manières différentes. Un des exemples les plus connus est \emph{le tri} : tri à
...
Le Machine Learning se découpe en deux approches : l'apprentissage supervisé (\emph{Supervised learning}) et l'apprentissage non supervisé
...
exactement \emph{au mapping} : c'est un ensemble de règles que prend un problème en entrée et qui possède une réponse. Ce modèle peut être très simple (une règle seule et simple)
tout comme il peut être arbitrairement complexe (combinaisons de jeux de règles complexes et inter-indépendants). On peut également dire qu'un modèle est la solution à un en problème
...
avec \emph{un ensemble de valeurs numériques} représentant la réponse à cette instance spécifique.
...
\paragraph{Classification et Regression}
